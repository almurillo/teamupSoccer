			...
			<div class="card">
				...
				<table class="table">
					...
					<tr>
						...
						<td>{{ $game->place }}</td>
						<td>{{ $game->field }}</td>
						<td>{{ date('m/d/Y h:i A', strtotime($game->game_time)) }}</td>
						<td><a href="#" class="btn btn-sm btn-success"><i class="fa fa-user"></i> 5</a></td>
						<td>
							
									<form action="{{ route('player.store', [$game->team_id, $game->id]) }}" method="POST">
										{{ csrf_field() }}
										<button class="btn btn-sm btn-default btn-block" type="submit" name="addPlayer" title="Join"><i class="fa fa-soccer-ball-o"></i> Join</button>
									</form>
			</div>
								...
								<td>
									...
									<a href="{{ route('games.edit',[$game->team_id, $game->id]) }}" class="btn btn-sm btn-primary btn-block"><i class="fa fa-edit"></i> Edit</a>
								</td>
								

								<td>
									
									<a href="{{ route('games.destroy',[$game->team_id, $game->id]) }}" class="btn btn-sm btn-danger btn-block"><i class="fa fa-trash"></i> Delete</a>
		...
		<div class="row">
			...
			@foreach($approves as $approve)
				...
				<div class="col-sm-2">
					...
					<a class="text-center" href="#"><h4>{{ $approve->name }}</h4></a>
